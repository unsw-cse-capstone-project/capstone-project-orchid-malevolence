...
<template>
	<div class="box">
		...
		<textarea class='text_area' :disabled="isAble" v-model.lazy="content"
		          placeholder="want do u want to say"></textarea>
		<el-row>
			<el-button class='button' :disabled="isAble" @click="add" size="small"><span
							style="width:70px; height: 30px; display: inline-block; font-size: 15px; text-align: center">commit</span>
			</el-button>
		</el-row>
		...
		<ul class='list-group'>
			<li class='list-group-item' style="position: relative" v-for="item in items" :key="item.id">
				...
				<img @click="changeNumber"
				     style="display:inline-block ; position: absolute; right: 30px; top: 10px; width: 20px;height: 20px; text-align: center"
				     :src="imgUrl" alt="">
